...
def execute_pi(fname, echo_name, args):
    ...
    try:
        ...
        print(
            ">>>>> Syntax error occured when parsing PI file: '{}' <<<<<\n".format(
                fname
            )
        )
        print(err)
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--single", action="store_true", help="Only follow one execution possibility"
    )
    argparser.add_argument(
        "--color", action="store_true", help="Color output even when not a TTY"
    )
    argparser.add_argument(
        "--seed",
        type=int,
        default=0,
        help="When using --single, pick a consistent seed for which path is taken",
    )

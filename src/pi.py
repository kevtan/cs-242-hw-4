...
class Prog:
    def __init__(self, defns: Sequence["Defn"], procs: Sequence["Proc"]):
        self.defns, self.ps = defns, procs
...
class Proc(object):
    ...
    def _pp(self, l: int = 0) -> str:
        ...
        if isinstance(p, Parallel):
            ...
            return "[{} {} {}]. {}".format(
                p.s1, "=" if p.is_eq else "!=", p.s2, p.p._pp(2)
            )
...
class Send(Proc):
    ...
    def _tuple(self):
        return ("1_Send", self.c, self.x, self.p)
...
class Receive(Proc):
    ...
    def _tuple(self):
        return ("2_Receive", self.c, self.x, self.p)
...
class Equality(Proc):
    ...
    def _tuple(self):
        return ("4_Equality", self.s1, self.s2, self.p, self.is_eq)
...
def _sep(s: str, ps: Sequence[Proc], color: bool, l: int, this_l: int) -> List[str]:
    ...
    parens = l > this_l
    one_line = len(prevs) == 1 or (
        all(len(prev_l) == 1 for prev_l in prevs)
        and sum(len(prev_l[0]) for prev_l in prevs) < 80
    )
    # one_line = False
    if one_line:
